...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUIVisibility() {
        if (isPanelExpanded) {
            ...
            const gridClasses = ["md:grid-cols-3", "lg:grid-cols-4", "xl:grid-cols-5"];
            ...
            propertyListContainer.classList.add("grid", "gap-4", ...singleColGridClasses);
            ...
            propertyListContainer.classList.remove("grid", "gap-4", ...singleColGridClasses);
    ...
    function closeAllRightPanels() {
        Object.values(allPanels).forEach((p) => {
            ...
            p.isOpen = false;
            p.panel.style.right = isRightPanelOpen ? `${RIGHT_SIDE_PANEL_WIDTH}px` : `-${p.width}px`;
        });
    ...
    function openRightPanel(panelName) {
        ...
        searchBarContainer.style.right = `${panel.width + RIGHT_SIDE_PANEL_WIDTH + MARGIN}px`;
        ...
        if (panelName === "notification" && typeof renderNotifications === 'function') renderNotifications();
        if (panelName === "favorite" && typeof renderFavoriteProperties === 'function') renderFavoriteProperties();
        if (panelName === "compare" && typeof renderCompareGroups === 'function') renderCompareGroups();
