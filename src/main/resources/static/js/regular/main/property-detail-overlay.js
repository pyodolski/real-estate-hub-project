          ...
          <div class="p-6 space-y-6">
            ...
            <div class="flex border-b border-gray-200 mb-4 relative">
                ...
                <div class="flex-1 relative">
                    <button id="tab-prediction-${suffix}" class="w-full py-2 text-gray-500 hover:text-gray-700 font-medium transition-colors" onclick="window.switchDetailTab('${suffix}', 'prediction')">시세예측</button>
                    <!-- 허위매물 위험 말풍선 -->
                    <div id="anomaly-alert-balloon-${suffix}" class="hidden absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-max z-20">
                        <div class="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded shadow-lg relative">
                            허위매물 위험
                            <div class="absolute top-full left-1/2 transform -translate-x-1/2 border-4 border-transparent border-t-red-500"></div>
                        </div>
                    </div>
                </div>
...
window.displayPriceEstimation = function (data, suffix) {
  ...
  const isAnomaly = overlay?.dataset?.anomalyAlert === 'true';
      ...
      warningHtml = `
        <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-red-800">주의 필요</h3>
                    <div class="mt-2 text-sm text-red-700">
                        <p>주변 매물과 10프로 이상 가격차이가 나는 매물이므로 주의가 필요합니다.</p>
                    </div>
                </div>
            </div>
        </div>
