...
| 1.1.1  |||신청자 정보 입력 폼을 통해 이름, 연락처, 매물과의 관계을 입력한다.|이름/연락처/매물과의 관계|"홍길동"/"010-1234-5678"/"소유자"|null/"010-123", "abcd", "+82-10-1234"/null||H|P|
| 1.1.2  |||지도에서 주소 검색 후 좌표 선택 시 주소가 자동 조회되며, 주소 검색 시 지도 API를 통해 해당 위치가 표시된다.|주소 검색어/좌표 선택|"서울특별시 강남구 테헤란로 10"/지도 클릭 좌표(37.12345, 127.12345)|"%^@#", "", "아무것도 없는 주소"/null|4.1.4|H|P|
| 1.1.3  |||소유권 증명 서류 업로드 시 파일 형식과 크기를 검증하고, 저장 시 파일명과 업로드 시간이 기록된다.|파일 업로드|"등기부등본.pdf (2MB)"|"exe 파일", "100MB 파일"|5.1.2, 5.1.4|H|P|
| 1.1.4  |||필수 입력값(이름, 연락처, 주소, 파일 등)이 올바른 형식이 아닐 경우 오류 메세지를 출력한다.|모든 필수 입력값|형식 적합한 모든 값|누락 값(이름 빈 칸 등)||H|P|
| 1.1.5  |||연락처 형식이 잘못되거나 파일 크기 제한을 초과하면 에러 메시지를 출력한다.|연락처/파일크기|"01012345678"/5MB 이하|문자 포함, 짮은 번호/20MB||H|P|
|   2    |매물 신청 현황 조회|||||||||
|  2.1   ||사용자가 신청한 매물들의 상태를 확인한다.|||||||||
| 2.1.1  |||관리자를 통해 승인된 신청은 '승인됨'으로 표시한다.|UI 버튼(조회)|승인된 신청 데이터 존재|없음|7.1.3|H|P|
| 2.1.2  |||신청 상태가 '심사중'이면 남은 일수, '거절됨'이면 거절 사유를 함께 표시한다.|상태 값|"심사중","거절됨"|null|7.1.4|H|P|
| 2.1.3  |||신청에 첨부된 서류는 원본 파일명으로 다운로드 할 수 있다.|다운로드 버튼|실제 파일명.pdf|파일 존재X|5.1.4|H|P|
|   3    |매물 관리 현황 요약|||||||||
|  3.1   ||전체 매물 관리 상황을 한눈에 파악할 수 있도록 요약 정보를 제공한다.|||||||||
| 3.1.1  |||각 상태별 매물 건 수를 계산하여 사용자에게 제공한다.|대시보드 로딩|데이터 존재||8.1.2|H||
