...
| 11.1.1 |            |                                     |사용자 유형은 reqular, broker, admin 중 하나를 선택한다.| role | `regular`, `broker`, `admin` | `guest`, `""`, `null` | 15.1.1 | H | P |
| 11.1.2 |            |                                     |broker 선택 시 licenseNumber를 필수로 기입하고 agencyName을 선택적으로 기입한다.| role, licenseNumber, agencyName | `role="broker"`, `licenseNumber="11-123-45678"`, `agencyName` 유/무 | `role="broker"` 이면서 `licenseNumber` 누락/빈 값, `role="regular"` 인데 `licenseNumber` 포함 | 15.1.2 | H | P |
| 11.1.3 |            |                                     |이메일, 사용자명, 비밀번호(8~64자)의 입력값을 검증한다.| email, username, password | `email="user1@test.com"`, 8~64자 비밀번호(`"Abcd1234!"`) | 잘못된 이메일 형식, 7자 이하 또는 65자 이상 비밀번호 | 14.1.5 | H | P |
| 11.1.4 |            |                                     |이미 등록된 이메일일 경우 오류를 반환한다.| email | 미등록 이메일(`"unique@test.com"`) | 이미 등록된 이메일(`"dup@test.com"`) |        | H | P |
| 11.1.5 |            |                                     |회원가입 시 최대 30개의 중복 불가인 태그 등록이 가능하고 없는 태그는 DB에서 자동으로 생성한다.| tags[] | 0~30개, `groupCode`/`keyCode` 중복 없음 | 31개 이상, 동일 `groupCode+keyCode` 중복, 공백 `groupCode`/`keyCode` | 15.2.3 | H | P |
| 11.1.6 |            |                                     |회원가입 완료 시 사용자/중개사 프로필(broker일 경우)/ 태그를 저장한다.| SignupRequest 전체 | 필수 필드(email, username, password, role) + 선택 필드(intro, phoneNumber, tags) 정상 입력 | 필수 필드 누락 요청 | 15.1.1, 15.1.2 | H | P |
| 11.1.7 |            |                                     |회원가입 성공 시 로그인 페이지로 이동한다.| 화면 동작(회원가입 버튼) | 회원가입 201 응답 후 로그인 페이지로 라우팅 | 회원가입 실패(400/409)인데도 로그인 페이지로 이동 또는 아무 이동 없음 | 12.1.1 | H | P |
|   12   |로그인 / 토큰 관리|                                     |                                           |          |            |            |        |        |            |
|  12.1  |            |사용자가 로그인 후 토큰을 발급받아 인증을 유지한다.    |                                           |          |            |            |        |        |            |
| 12.1.1 |            |                                     |이메일과 비밀번호로 로그인을 시도한다.| email, password | `/api/auth/login` 에 존재하는 계정의 이메일·비밀번호 | 잘못된 비밀번호, 존재하지 않는 이메일 | 11.1.3 | H | P |
| 12.1.2 |            |                                     |로그인 성공 시 AccessToken(JWT)와 RefreshToken을 발급한다.| email, password (정상 로그인 요청) | 응답 바디에 `accessToken` 존재, 응답 헤더에 `rt` HttpOnly 쿠키 생성 | 자격 증명 오류 시에도 토큰/쿠키가 발급되는 경우 |        | H | P |
| 12.1.3 |            |                                     |rememberMe 옵션에 따라 RefreshToken 만료일을 14일과 30일 중 결정한다.| rememberMe | `false` → 만료일 14일, `true` → 만료일 30일 | 잘못된 타입, null 로 인한 의도치 않은 만료일 설정 |        | H | P |
| 12.1.4 |            |                                     |로그인 실패 시에 에러 메세지를 반환한다.| email, password | 잘못된 조합으로 로그인 시 4xx + `"이메일 또는 비밀번호가 올바르지 않습니다."` | 실패인데도 200 OK + 토큰 발급 |        | H | P |
| 12.1.5 |            |                                     |로그인 성공 시 사용자의 역할에 따라 메인 페이지가 분기된다.| role | `role="regular"` → 사용자 메인, `role="broker"` → 브로커 대시보드, `role="admin"` → 관리자 페이지 | DB에 `role="unknown"` 인 계정으로 로그인 시 목적지 없는 라우팅/에러 | 11.1.1 | H | P |
| 12.1.6 |            |                                     |RefreshToken을 이용해 새로운 AccessToken 재발급이 가능하다.| rt 쿠키 또는 refreshToken 파라미터 | 유효한 `RefreshToken` 으로 `/api/auth/refresh` 호출 시 새 `accessToken` 발급 | 만료·revoked 토큰으로 호출 시에도 새 토큰 발급 |        | H | P |
| 12.1.7 |            |                                     |만료/폐기된 RefreshToken 사용 시 오류를 반환한다.| rt 쿠키 또는 refreshToken 파라미터 | 만료/폐기된 토큰 사용 시 `"유효하지 않은 리프레시 토큰"` 오류 반환 | 만료/폐기된 토큰인데 정상 발급 | 13.1.1 | H | P |
|   13   |로그아웃|                                     |                                           |          |            |            |        |        |            |
|  13.1  |            |사용자가 로그아웃하여 인증 세션을 종료한다.|                                           |          |            |            |        |        |            |
| 13.1.1 |            |                                     |RefreshToken을 revoked 처리하여 무효화한다.| rt 쿠키, AuthUser.id | 로그인 상태에서 `/api/auth/logout` 호출 시 해당 `RefreshToken.revoked=true` | DB에 없는 rt 값, 다른 사용자 토큰인 경우 → 변경 없음 | 12.1.2 | H | P |
| 13.1.2 |            |                                     |브라우저 쿠키와 LocalStorage/SessionStorage의 토큰을 삭제한다.| 로그아웃 버튼, 클라이언트 저장소 토큰 | 로그아웃 후 쿠키(rt), `localStorage.accessToken`, `sessionStorage.accessToken` 제거 | 저장소에 토큰이 남아 새로고침 후에도 로그인 상태처럼 보이는 경우 | 12.1.2 | H | P |
| 13.1.3 |            |                                     |로그아웃 완료 후 재인증 없이는 API 요청이 불가하다.| Authorization 헤더 | 로그아웃 후 토큰 없이 `/api/users/me` 호출 시 401/403 | 로그아웃 후에도 이전 토큰으로 보호 API 호출이 성공 | 12.1.6 | H | P |
|   14   |비밀번호 재설정|                                     |                                           |          |            |            |        |        |            |
|  14.1  |            |비밀번호를 분실한 사용자가 비밀번호를 재설정한다.|                                           |          |            |            |        |        |            |
| 14.1.1 |            |                                     |비밀번호 찾기 요청 시 PasswordResetToken을 발급하고 비밀번호 재설정 링크가 담긴 이메일을 발송한다.| email | 존재하는 이메일로 `/api/auth/forgot-password` 호출 시 토큰 생성 + 메일 전송 | 존재하지 않는 이메일 → 응답은 204지만 토큰/메일 생성 없음 | 12.1.2 | H | P |
| 14.1.2 |            |                                     |토큰은 1시간 동안 유효하고, 만료 시 재사용이 불가하다.| PasswordResetToken.token, expiresAt | 발급 후 1시간 이내 `/api/auth/reset-password` 사용 시 성공, 1시간 이후 사용 시 만료 에러 | 만료·used 토큰으로도 비밀번호 변경 성공 | 12.1.7 | H | P |
| 14.1.3 |            |                                     |메일 링크를 클릭 시 비밀번호 재설정 화면으로 이동한다.| 메일 내 reset 링크 클릭 | 메일의 `<a href=".../reset-password?token=xxx">` 클릭 시 `/reset-password.html?token=xxx` 로 리다이렉트 | 잘못된 토큰 값으로 접근 시 에러 안내 화면 |        | H | P |
| 14.1.4 |            |                                     |새 비밀번호 입력 시 토큰 유효성 검증 후 비밀번호가 변경된다.| token, newPassword | 유효 토큰 + 8~64자 새 비밀번호로 `/api/auth/reset-password` 호출 시 `passwordHash` 변경 및 토큰 used 처리 | 존재하지 않거나 만료/used 토큰으로 호출 시 `"유효하지 않은 토큰입니다."`/만료 에러 | 15.2.5 | H | P |
| 14.1.5 |            |                                     |새 비밀번호 또한 8~64자로 제한되며, 기존과 동일 시 오류를 반환한다. | newPassword, currentPassword | `/me/change-password` 에서 8~64자이며 기존과 다른 새 비밀번호 사용 | 기존과 동일한 비밀번호 사용 또는 길이(7자 이하/65자 이상) 위반 | 15.2.4 | H | P |
| 14.1.6 |            |                                     |비밀번호 변경이 완료되면 로그인 페이지로 이동한다.| 화면 동작(비밀번호 재설정 완료) | 재설정 성공 후 로그인 페이지로 이동(또는 로그인 유도 버튼) | 재설정 실패인데도 로그인 페이지로 이동하는 등 오동작 | 12.1.1 | H | P |
|   15   |사용자 프로필 관리|                                     |                                           |          |            |            |        |        |            |
|  15.1  |            |사용자가 자신의 프로필 정보를 조회한다.|                                           |          |            |            |        |        |            |
| 15.1.1 |            |                                     |내 정보 조회 시 이메일, 닉네임, 역할, 전화번호, 소개글, 사진, 태그를 반환한다.| Authorization 헤더(accessToken) | 유효한 accessToken 으로 `/api/users/me` 호출 시 필드 전체 반환 | 토큰 없음/만료 토큰으로 호출 시 401/403 | 12.1.2 | H | P |
| 15.1.2 |            |                                     |broker 계정인 경우 licenseNumber, agencyName, intro, profileImageUrl을 포함한다.| role=broker, 브로커 상세 조회 | 브로커 계정으로 `/api/users/me` + `/api/brokers/{userId}` 호출 시 라이선스/소속/프로필 정보 조회 | role=regular 인 계정에서 브로커 정보까지 노출되거나, 비활성/비브로커 계정에 대해 200 응답 | 11.1.2 | H | P |
|  15.2  |            |사용자가 자신의 프로필 및 태그를 수정한다.|                                           |          |            |            |        |        |            |
| 15.2.1 |            |                                     |프로필 수정 시 현재 비밀번호 검증을 필요로 한다.| currentPassword | `/api/users/me` PUT 시 현재 비밀번호를 정확히 전달하면 수정 성공 | 잘못된 currentPassword 로 요청 시 `"현재 비밀번호가 일치하지 않습니다."` | 12.1.1 | H | P |
| 15.2.2 |            |                                     |사용자는 프로필의 태그, 사진, 비밀번호, 프로필 소개문을 수정할 수 있다.| phoneNumber, profileImageUrl, intro, tags | 각 필드를 단독/조합으로 수정 요청 시 정상 반영 | 권한 없는 필드(예: role 변경) 시도, 잘못된 타입/형식으로 인한 Validation 에러 | 15.1.1, 15.1.2 | H | P |
| 15.2.3 |            |                                     |태그는 수정 시 전체 삭제 후 재등록한다. (최대 30개, 중복 불가, 없으면 DB 생성)| tags[] | 태그 0~30개, 중복 없는 `groupCode`/`keyCode` 로 재등록 | 31개 이상, 동일 `groupCode+keyCode` 중복 태그 | 40.1.3 | H | P |
| 15.2.4 |            |                                     |비밀번호 변경 시에는 기존 비밀번호 검증 및 동일 여부를 확인한다.| currentPassword, newPassword | `/api/users/me/change-password` 에서 현재 비밀번호 일치 + 새 비밀번호가 기존과 다른 값 | 현재 비밀번호 불일치, 새 비밀번호가 기존과 동일 | 12.1.1 | H | P |
| 15.2.5 |            |                                     |비밀번호 변경 성공 시 새로운 해시로 저장한다.| newPassword | 비밀번호 변경 후 `passwordHash` 가 이전과 다른 해시 값으로 저장 | 평문 저장, 해시 미변경 |        | H | P |
|   16   |위임 관리|                                     ||||||     |       |
