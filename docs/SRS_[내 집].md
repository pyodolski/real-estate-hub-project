...
| 1.1.2  |||지도에서 주소 검색 후 좌표 선택 시 주소가 자동 조회되며, 주소 검색 시 지도 API를 통해 해당 위치가 표시된다.|주소 검색어/좌표 선택|"서울특별시 강남구 테헤란로 10"/지도 클릭 좌표(37.12345, 127.12345)
|"%^@#", "", "아무것도 없는 주소"/null|4.1.4|H|P|
| 1.1.3  |||소유권 증명 서류 업로드 시 파일 형식과 크기를 검증하고, 저장 시 파일명과 업로드 시간이 기록된다.|파일 업로드|"등기부등본.pdf (2MB)"|"exe 파일", "100MB 파일"|5.1.2, 5.1.4|H|P|
| 1.1.4  |||필수 입력값(이름, 연락처, 주소, 파일 등)이 올바른 형식이 아닐 경우 오류 메세지를 출력한다.|모든 필수 입력값|형식 적합한 모든 값|누란 값(이름 빈 칸 등)||H|P|
